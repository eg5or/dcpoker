...
export function CardFront({ user, isCurrentUser, hasVoted, isRevealed, style }: CardFrontProps) {
  return (
    <div
    ...
    >
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <h3
          className={`text-lg sm:text-xl truncate max-w-[80%] ${isCurrentUser ? 'text-blue-300 font-bold' : 'text-white'}`}
        >
          {isCurrentUser ? `${user.name} (Вы)` : user.name}
        </h3>
        <div>
          <span
            className={`h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full inline-block ${
