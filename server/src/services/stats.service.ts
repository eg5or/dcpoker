...
export class StatsService {
    ...
    try {
      ...
      if (targetId !== senderId) {
        ...
        if (targetStats) {
          ...
          const receivedEmojiIndex = targetStats.emojisStats.received.findIndex(
            (e) => e.emoji === emoji
          );
  ...
  static async updateSessionStats(sessionId: string): Promise<void> {
    try {
      ...
      console.log(
        `Сессия ${sessionId} содержит ${votesWithChanges.length} измененных голосов после раскрытия`
      );
      ...
      for (const vote of session.votes) {
        ...
        const userIdStr = typeof vote.userId === 'string' ? vote.userId : vote.userId.toString();
        ...
        try {
          ...
          if (vote.changedAfterReveal) {
            console.log(
              `Пользователь ${userIdStr} изменил голос после раскрытия: ${vote.initialVote} -> ${vote.finalVote}`
            );
  ...
  static async updateCompletedSessionStats(sessionId: string): Promise<void> {
    try {
      ...
      if (globalStats) {
        ...
        console.log(
          `Обновлена статистика завершенных сессий для ${sessionId}. Всего завершено: ${globalStats.completedSessions}`
        );
  ...
  private static async updateGlobalSessionStats(session: any): Promise<void> {
    try {
      ...
      const validVotes = session.votes.filter(
        (vote: any) => vote.initialVote !== null && typeof vote.initialVote === 'number'
      );
      ...
      for (const vote of validVotes as Array<{ initialVote: number }>) {
        ...
        const voteIndex = globalStats.votesStats.values.findIndex(
          (v: { value: number }) => v.value === voteValue
        );
      ...
      console.log(
        `Глобальная статистика обновлена для сессии ${sessionId}. Всего сессий: ${globalStats.totalSessions}`
      );
  ...
  static async updateVoteChangesStats(sessionId: string): Promise<void> {
    try {
      ...
      const changedVotes = session.votes.filter((vote: any) => vote.changedAfterReveal);
      console.log(
        `Обновление статистики для ${changedVotes.length} измененных голосов в сессии ${sessionId}`
      );
      ...
      changedVotes.forEach((vote: any, index: number) => {
        ...
        console.log(
          `[${index}] Изменённый голос: userId=${userId}, username=${vote.username}, initialVote=${vote.initialVote}, finalVote=${vote.finalVote}, changedAfterRevealCounted=${vote.changedAfterRevealCounted}`
        );
      });
      ...
      for (const vote of changedVotes) {
        ...
        try {
          ...
          if (!userStats) {
            ...
            console.log(
              `Найдена существующая статистика для пользователя ${userIdStr}: changedAfterReveal=${userStats.votesStats.changedAfterReveal}`
            );
          }
          ...
          if (!vote.changedAfterRevealCounted) {
            console.log(
              `Увеличиваем счетчик изменений для пользователя ${userIdStr}: ${userStats.votesStats.changedAfterReveal} -> ${userStats.votesStats.changedAfterReveal + 1}`
            );
            userStats.votesStats.changedAfterReveal += 1;
            ...
            console.log(
              `Установлен флаг changedAfterRevealCounted для голоса пользователя ${userIdStr}`
            );
          } else {
          ...
          await userStats.save();
          console.log(
            `Статистика пользователя ${userIdStr} успешно сохранена, текущее значение changedAfterReveal=${userStats.votesStats.changedAfterReveal}`
          );
        } catch (error) {
          console.error(
            `Ошибка при обновлении статистики изменённых голосов для пользователя ${userIdStr}:`,
            error
          );
          // Продолжаем с другими пользователями, не прерывая процесс
        }
      ...
      if (globalStats) {
        ...
        if (!processedSessionIds.includes(sessionId)) {
          console.log(
            `Сессия ${sessionId} еще не учтена в глобальной статистике, обновляем общие счетчики`
          );
          ...
          console.log(
            `Глобальная статистика обновлена для новой сессии ${sessionId}. Всего сессий: ${globalStats.totalSessions}`
          );
        }

        // Подсчитываем новые изменённые голоса, которые еще не были учтены
        const uncountedChanges = changedVotes.filter(
          (vote: any) => !vote.changedAfterRevealCounted
        ).length;

        console.log(
          `Всего ${changedVotes.length} измененных голосов, из них ${uncountedChanges} еще не учтены в глобальной статистике`
        );

        if (uncountedChanges > 0) {
          console.log(
            `Обновляем глобальную статистику изменений голосов: ${globalStats.votesStats.changedAfterReveal} -> ${globalStats.votesStats.changedAfterReveal + uncountedChanges}`
          );
